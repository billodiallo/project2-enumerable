...
describe Enumerable do
    ...
    describe '#my_select' do
        ...
        it 'returns the items that meet a given condition' do
            expect((1..10).my_select {|i| i % 2 == 0}).to eq([2, 4, 6, 8, 10])
        end
    end

    describe '#my_all?' do
        it 'returns true if all the words are equal to or greater than three' do
            expect(['cat', 'dog', 'bear'].my_all? {|i| i.length >= 3}).to eq(true)
        end
        it 'returns false if all the words are equal to or greater than four' do
            expect(['cat', 'dog', 'bear'].my_all? {|i| i.length >= 4}).to eq(false)
        end
        ...
        it 'returns true if all the elements match a specific pattern' do
            expect(['cat', 'dog', 'bear'].my_all?(/a/)).to eq(false)
            expect(['cat', 'bag', 'bear'].my_all?(/a/)).to eq(true)
        end
    end

    describe '#my_any' do
        it 'returns true if any of the elements length exceeds 4' do
            expect(['cat', 'dog', 'bear'].my_any? {|word| word.length >= 4}).to eq(true)
        end
        it 'returns true if any of the elements meet a specific pattern' do
            expect(['cat', 'dog', 'bear'].my_any?(/t/)).to eq(true)
            expect(['cat', 'dog', 'bear'].my_any?(/z/)).to eq(false)
        end
    ...
    describe '#my_none' do
        ...
        it 'returns true if no word has a word length of 5' do
            expect(['cat', 'dog', 'bear'].my_none? {|word| word.length == 5}). to eq(true)
        end
        it 'returns true when none of the element has /z/' do
            expect(['cat', 'dog', 'bear'].my_none?(/z/)).to eq(true)
        end
    end
